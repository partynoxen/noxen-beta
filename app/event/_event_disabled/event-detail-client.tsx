...
export default function EventDetailClient({ eventId }: { eventId: string }) {
  ...
  const handleCreatePlan = () => {
    try {
      ...
      // Erstmal sicher zurück zur Startseite.
      // Wenn /plan stabil existiert, später wieder auf /plan ändern.
      router.push('/');
    } catch (error) {
  ...
  if (!event) {
    return (
      <div className="min-h-dvh bg-[#050509] flex flex-col items-center justify-center pb-20 px-6 text-center">
        <p className="text-white/50 mb-4">Event nicht gefunden</p>
        <button
          onClick={() => router.back()}
      ...
      <div className="max-w-lg mx-auto px-4 -mt-16 relative z-10">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass rounded-2xl p-4 mb-4">
            <HeatScore score={event.heatScore ?? 0} size="lg" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            <div className="glass rounded-xl p-3">
              <div className="flex items-center gap-2 text-blue-400 mb-1">
                <Clock size={14} />
                <span className="text-xs font-semibold">Uhrzeit</span>
              </div>
            ...
            <div className="glass rounded-xl p-3 col-span-2">
              ...
              <p className="text-sm text-white/80">
                {event.location ?? ''}, {displayCity(event.city ?? '')}
              </p>
            </div>
          </div>
          ...
          {lineupArr.length > 0 && (
            <div className="glass rounded-2xl p-4 mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {lineupArr.map((dj: string, i: number) => (
                  <span
                    key={`${dj}-${i}`}
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold hover:from-purple-500 hover:to-pink-500 transition-all"
              onClick={(e) => {
                if (!event.ticketUrl) {
                  e.preventDefault();
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 py-3 rounded-xl glass text-white/70 hover:bg-white/10 font-semibold transition-colors"
            >
              <Navigation size={18} />
              Navigieren
            </a>
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            ...
            <button
              onClick={() => alert('Taxi-Modul kommt als nächstes.')}
              className="flex items-center justify-center gap-2 py-3 rounded-xl glass text-white/70 hover:bg-white/10 font-semibold transition-colors"
            >
              <Car size={18} />
              Taxi holen
